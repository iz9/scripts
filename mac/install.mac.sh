...
run_script_in_new_tab() {
    ...
    echo "Press Enter when $script_path has finished executing..."
    read -r
}
...
main() {
    ...
    if [ "$LAST_STEP" -lt 1 ]; then
        run_script_in_new_tab "${SCRIPT_DIR}/install_xcode_tools.mac.sh"
        mark_step_completed "1"
    fi

    # Step 2: Homebrew
    if [ "$LAST_STEP" -lt 2 ]; then
        run_script_in_new_tab "${SCRIPT_DIR}/install_homebrew.mac.sh"
    ...
    if [ "$LAST_STEP" -lt 3 ]; then
        run_script_in_new_tab "${SCRIPT_DIR}/install_rosetta.mac.sh"
        mark_step_completed "3"
    fi

    # Step 4: Dev libs
    if [ "$LAST_STEP" -lt 4 ]; then
        run_script_in_new_tab "${SCRIPT_DIR}/install_dev_libs.mac.sh"
    ...
    if [ "$LAST_STEP" -lt 5 ]; then
        run_script_in_new_tab "${SCRIPT_DIR}/install_chrome.mac.sh"
        mark_step_completed "5"
    fi

    # Step 6: Git
    if [ "$LAST_STEP" -lt 6 ]; then
        run_script_in_new_tab "${SCRIPT_DIR}/git.mac.sh"
    ...
    if [ "$LAST_STEP" -lt 7 ]; then
        run_script_in_new_tab "${SCRIPT_DIR}/install_manual.mac.sh"
        mark_step_completed "7"
    fi

    # Step 8: Fonts
    if [ "$LAST_STEP" -lt 8 ]; then
        run_script_in_new_tab "${SCRIPT_DIR}/fonts.mac.sh"
        mark_step_completed "8"
    fi

    # Step 9: Shell config
    if [ "$LAST_STEP" -lt 9 ]; then
        run_script_in_new_tab "${SCRIPT_DIR}/config_shell.mac.sh"
    ...
    if [ "$LAST_STEP" -lt 10 ]; then
        run_script_in_new_tab "${SCRIPT_DIR}/install_cli_tools.mac.sh"
    ...
    if [ "$LAST_STEP" -lt 11 ]; then
        run_script_in_new_tab "${SCRIPT_DIR}/nvim.mac.sh"
        mark_step_completed "11"
    fi

    # Step 12: Wezterm
    if [ "$LAST_STEP" -lt 12 ]; then
        run_script_in_new_tab "${SCRIPT_DIR}/wezterm.mac.sh"
        mark_step_completed "12"
    fi

    # Step 13: GUI apps
    if [ "$LAST_STEP" -lt 13 ]; then
        run_script_in_new_tab "${SCRIPT_DIR}/install_gui_apps.mac.sh"
        mark_step_completed "13"
    fi

    # Step 14: ASDF
    if [ "$LAST_STEP" -lt 14 ]; then
        run_script_in_new_tab "${SCRIPT_DIR}/install_asdf.mac.sh"
    ...
    if [ "$LAST_STEP" -lt 15 ]; then
        ...
        read -r choice
        run_script_in_new_tab "${SCRIPT_DIR}/dotbot.mac.sh $choice"
